...
@von_rueden_leadership_2015 outilne four possible mechanisms by which leaders in small-scale societies might offset the potential costs associated with various leadership roles; leaders may, (1) facilitate collective actions that yield net benefits to themselves or their close kin, (2) claim a fee for their services, such a tax or greater share of returns, (3) recieve reciprocal exchange in other currencies from followers who have benefited from their services, or (4) may gain other social or reproductive benefits by signaling their high quality as a mate and social partner.
...
Table: (\#tab:qualfuncvars) Operational definitions for coded leader qualities and functions. 

```{r qualfuncvars, echo=FALSE, message=F, warning=FALSE}
library(readr)
qual_func_var_tab<-read_csv('func_qual_definitions.csv', na = 'NA')
library(knitr)
kable(qual_func_var_tab)
```
...
Table: (\#tab:costbenefitvars) Operational definitions for coded benefits and costs for leaders and followers.

```{r costbenefitvars, echo=FALSE, message=F, warning=FALSE, }
library(readr)
cost_benefit_vars_tab<-read_csv('costs_benefits.csv', na = 'NA')
library(knitr)
kable(cost_benefit_vars_tab)
```
...
Leaders are most commonly receiving benefits in the form of greater access to resources and increased social status. Receiving social services and mating benefits are also relatively frequent benefits of leadership cross-culturally. The costs follow the potential benefits and leaders stand to lose their high social status and greater access to resources. Followers on the other hand most commonly benefit from leadership systems through receiving resources and social services. Mating and fitness benefits are relatively rare for followers. Followers most frequently incur costs in the form of resources lost and in providing social services. For both leaders and followers, individuals stand to lose what they are able to gain. The clear differences lie in leaders' opportunities for increased social status and matting opportunities. See Figure \@ref(fig:varsupportcb).
